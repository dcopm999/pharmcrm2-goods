...
def coverage(c):
    ...
    """
    c.run("coverage run --source pharmcrm2-goods runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pharmcrm2-goods.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ goods")
...
def lint(c):
    ...
    """
    c.run("flake8 pharmcrm2-goods tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=pharmcrm2_goods.__version__
        )
    )
    c.run("git push --tags")
